...
from PraNet.utils.eval_functions import *
...
def evaluate(opt):
    result_path = Path(workspace).joinpath(opt.Eval.result_path).as_posix()
    ...
    headers = (
        opt.Eval.metrics
    )  # ['meanDic', 'meanIoU', 'wFm', 'Sm', 'meanEm', 'mae', 'maxEm', 'maxDic', 'maxIoU', 'meanSen', 'maxSen', 'meanSpe', 'maxSpe']
    results = []

    if opt.Eval.verbose is True:
        print("#" * 20, "Start Evaluation", "#" * 20)
        datasets = tqdm.tqdm(
            opt.Eval.datasets,
            desc="Expr - " + method,
            total=len(opt.Eval.datasets),
            position=0,
    ...
    for dataset in datasets:
        pred_root = Path(workspace).joinpath(opt.Eval.pred_root, dataset).as_posix()
        gt_root = Path(workspace).joinpath(opt.Eval.gt_root, dataset, "masks")
...
if __name__ == "__main__":
    ...
    opt_config = {
        "Eval": {
            "verbose": True,
            "gt_root": "datasets/diffusion_datasets/2D_polyp_test_seperate",
            "pred_root": f"results/{args.model_name}/{EXP}",
            "result_path": f"results/{args.model_name}/{EXP}/eval_results",
            "datasets": [
                "CVC-300",
                "CVC-ClinicDB",
                "Kvasir",
    ...
    opt = OmegaConf.merge(opt_config)
