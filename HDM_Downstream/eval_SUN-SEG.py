...
from global_config import workspace, MODEL_NAME, EXP

# global params
GT_ROOT = Path(workspace).joinpath("datasets", "SUN-SEG").as_posix()
PRED_ROOT = Path(workspace).joinpath("results", MODEL_NAME).as_posix()
...
def evaluator(gt_pth_lst, pred_pth_lst, metrics):
    ...
    for metric_module_name in metric_module_list:
        metric_module[metric_module_name] = getattr(
            __import__("metrics", fromlist=[metric_module_name]), metric_module_name
                ...
                for case in case_list:
                    ...
                    try:
                        case_gt_name_list.sort(
                            key=lambda name: (
                                int(name.split("/")[-2]),
                                int(name.split("/")[-1].rstrip(".png")),
                            )
                        )
                    except:
                        case_gt_name_list.sort(
                            key=lambda name: (
                                int(name.split("/")[-2].split("case")[1].split("_")[0]),
                                0
                                if not len(name.split("/")[-2].split("_")) > 1
                                else int(name.split("/")[-2].split("_")[1]),
                                int(name.split("/")[-1].split("_a")[1].split("_")[0]),
                                int(name.split("/")[-1].split("_image")[1].split(".png")[0]),
                            )
                        )
                    ...
                    case_pred_name_list = [
                        gt.replace(gt_src, pred_src).replace(".png", ".jpg") for gt in case_gt_name_list
                    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gt_root",
        type=str,
        help="custom your ground-truth root",
        default=GT_ROOT,
    )
    parser.add_argument(
        "--pred_root",
        type=str,
        help="custom your prediction root",
        default=PRED_ROOT,
    )
    parser.add_argument(
        "--metric_list",
    ...
    parser.add_argument(
        "--data_lst",
        type=str,
        help="set the dataset what you wanna to test",
        nargs="+",
        action="append",
        choices=[
        ...
        default=[["TestUnseenDataset"]],
    )
    parser.add_argument(
        "--model_lst", type=str, help="candidate competitors", nargs="+", action="append", default=[[EXP]]
    )
    parser.add_argument("--txt_name", type=str, help="logging root", default=EXP)
    parser.add_argument(
        "--check_integrity",
        type=bool,
    ...
    txt_save_path = Path(workspace).joinpath("results", MODEL_NAME).joinpath(opt.txt_name).as_posix()
