...
def res2net50_v1b_26w_4s(pretrained=False, **kwargs):
    ...
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4, **kwargs)
    if pretrained:
        model_state_loaded = torch.load(os.path.join(workspace, "models/PraNet/res2net50_v1b_26w_4s-3cf99910.pth"))

        model_dict = model.state_dict()
        state_dict = {
            k: v for k, v in model_state_loaded.items() if k in model_dict.keys() and v.shape == model_dict[k].shape
        }
