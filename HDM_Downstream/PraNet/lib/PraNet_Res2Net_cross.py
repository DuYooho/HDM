...
from PraNet.lib.Res2Net_v1b import res2net50_v1b_26w_4s
...
class PraNet(nn.Module):
    ...
    def __init__(self, channel=32, **kwargs):
        ...
        in_channel = kwargs.pop("in_channel", 3)
        # ---- ResNet Backbone ----
        self.resnet = res2net50_v1b_26w_4s(pretrained=True, in_channel=in_channel)
        ...
        # cross attn
        self.proj = nn.Conv2d(3, 64, kernel_size=1, stride=1, padding=0)

        self.cross_attn = CrossAttention(query_dim=64)
...
if __name__ == "__main__":
    ...
    cond = torch.randn(1, 3, 32, 32).cuda()

    out = ras(input_tensor, cond)
    print(out[0].shape)
    print(out[1].shape)
    print(out[2].shape)
    print(out[3].shape)
